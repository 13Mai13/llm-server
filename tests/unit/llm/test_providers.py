...
async def test_generate_method(groq_provider):
    ...
    mock_response = {
        "id": "chatcmpl-123",
        "choices": [
            {"message": {"content": "This is a test response from the Groq API."}}
        ],
    ...
    with patch.object(
        groq_provider.client, "post", return_value=mock_response_obj
    ) as mock_post:
        # Call generate method
        response = await groq_provider.generate(
            model="llama3-8b-8192", prompt="Tell me a short story."
        )
...
async def test_generate_unsupported_model(groq_provider):
    ...
    with pytest.raises(
        ValueError, match="Model 'unsupported-model' not supported by Groq provider"
    ):
        await groq_provider.generate(
            model="unsupported-model", prompt="This should fail."
        )
    ...
    with patch.object(provider.client, "post", new_callable=AsyncMock) as mock_post:
        ...
        with pytest.raises(ValueError, match="Groq API error: 400 - Bad Request"):
            await provider.generate(
                model="llama3-8b-8192", prompt="Test error handling"
            )
