...
async def test_add_request_multiple(mock_batch_processor):
    ...
    try:
        ...
        tasks = [
            asyncio.create_task(manager.add_request(f"request_{i}"))
            for i in range(3)
        ]
...
async def test_cancellation():
    ...
    manager = BatchManager(
        batch_processor=slow_processor,
        max_batch_size=2,
        max_wait_time=0.1
    )
...
async def test_process_batches_error_handling():
    ...
    with patch("src.llm.batch_processing.logger") as mock_logger:
        ...
        try:
            ...
            mock_logger.error.assert_called_with("Error processing batch: Processor error")
