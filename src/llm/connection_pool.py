...
class ConnectionPool:
    ...
    def __init__(self, provider_name: str, max_connections: int = 10):
        ...
        logger.info(f"Created connection pool for {provider_name} with {max_connections} max connections")
    ...
    async def initialize(self, base_url: str, headers: Dict[str, str]) -> None:
        ...
        async with self.lock:
            ...
            logger.info(f"Initialized {len(self.connections)} connections for {self.provider_name}")
        ...
        async with self.semaphore:
            async with self.lock:
                if not self.available_connections:
                    logger.warning(f"No available connections for {self.provider_name}, creating a new one")
                    client = httpx.AsyncClient(
            ...
            try:
                ...
                async with self.lock:
                    if client in self.connections:  # It might have been removed if closed
                        self.available_connections.add(client)
    ...
    for provider_name, provider in providers.items():
        try:
            ...
            if not base_url or not api_key:
                logger.warning(f"Could not set up connection pool for {provider_name}: missing base_url or api_key")
                continue
            ...
            logger.error(f"Error setting up connection pool for {provider_name}: {str(e)}")
